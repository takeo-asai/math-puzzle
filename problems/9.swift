...
func isBalanced(sexes: [Sex]) -> Bool {
	for i in 1 ..< sexes.count - 1 {
		let a = sexes[0...i-1]
		let b = sexes[i...sexes.count-1]
		let a0 = a.reduce(0) {$0 + $1.rawValue}
		let b0 = b.reduce(0) {$0 + $1.rawValue}
		if a0 == 0 && b0 == 0 {
			return true
		}
	}
...
let males: [Sex] = Array(count: 20, repeatedValue: .Male)
let females: [Sex] = Array(count: 10, repeatedValue: .Female)

let group = males + females
// とても遅いが動く！30! > 10^32
